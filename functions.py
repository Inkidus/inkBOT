...
async def mediaAdd(originalMessage):
	async with originalMessage.channel.typing():
		...
		if not originalMessage.guild.voice_client:
			...
			await mediaAdd(originalMessage, mediaQueues)
			return
		...
		with yt_dlp.YoutubeDL(ydl_opts) as ydl:
			...
			if "entries" in info:
				...
				for entry in info["entries"]:
					...
					await add_to_queue(originalMessage, mediaQueues, audio_url, video_title, video_url)
		...
		if not originalMessage.guild.voice_client.is_playing():
			await play_next(originalMessage, mediaQueues)
...
async def mediaSkip(originalMessage):
	...
	await play_next(guild_id, mediaQueues)
...
async def chatForget(originalMessage, modelRequested):
	async with originalMessage.channel.typing():
		...
		if dictionaryKey in chatHistories:
...
async def chatSessionModify(originalMessage, modelRequested):
	...
	if user_content.lower().startswith('temperature'):
		...
		presenceWanted = int(user_content)
		...
		frequencyWanted = int(user_content)
...
async def chatPost(originalMessage, modelRequested):
	async with originalMessage.channel.typing():
		...
		if isPromptSafe:
			...
			completion_messages = chatHistories[dictionaryKey] + [{"role": "system", "content": time_of_response}] + [{"role": "user", "content": messageToSend}]
			...
			response = await generateCompletion(dictionaryKey, completion_messages, modelRequested, tempWanted, toppWanted, presenceWanted, frequencyWanted)
			
			isResponseSafe = await is_message_safe(response)

			# If the response passes moderation
			if isResponseSafe:
				
				# Log response
				logging.info(f"Response: {response}\n")
				
				# If GPT-4 Used, limit memory
				if ((len(chatHistories[dictionaryKey]) >= 12) and (modelRequested == "gpt-4")):
					for i in range(2):
						chatHistories[dictionaryKey].pop(2)
				
				# Store exchange to history and YAML file
				...
				# If custom profile wanted, send as custom profile
				...
				# Send response normally or using the long message function
...
async def play_next(originalMessage):
	...
	voice_client.play(FFmpegPCMAudio(executable="ffmpeg", source=audio_url, **FFMPEG_OPTIONS), after=lambda e: asyncio.run_coroutine_threadsafe(play_next(originalMessage, mediaQueues), voice_client.loop))
